...
- [x] `week-6` `type:chore` `p3` "More Work" expand affordance — pagination/expand pattern for the Work grid, to use once project count grows past the current 5 cards (future-proofing, not urgent yet) -- `VISIBLE_COUNT = 6` threshold added to Work.astro with a "More work" toggle button that only renders once `projects.length` exceeds it; with the current 5 projects the button stays hidden, so no visible change yet.

...
- [x] `week-6` `type:chore` `p3` "More Work" expand affordance — pagination/expand pattern for the Work grid, to use once project count grows past the current 5 cards (future-proofing, not urgent yet) -- already shipped and wired in PR #72 (VISIBLE_COUNT threshold + toggle button in Work.astro, confirmed functional in source); checkbox regressed after a later merge, re-checked here

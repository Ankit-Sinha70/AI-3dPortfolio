...
- [x] `week-6` `type:chore` `p3` "More Work" expand affordance — pagination/expand pattern for the Work grid, to use once project count grows past the current 5 cards (future-proofing, not urgent yet) -- already shipped: VISIBLE_COUNT threshold + more-toggle button + expand script in src/components/Work.astro; checkbox reconciled here

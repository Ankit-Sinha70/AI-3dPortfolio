...
void main(){vec3 n=normalize(vNormal);vec3 light=normalize(uLightDir);float ndl=max(dot(n,light),0.);float wrapped=max((dot(n,light)+.3)/1.3,0.);float rim=pow(1.-max(dot(n,normalize(cameraPosition-vWorldPosition)),0.),3.2);float macro=fbm(n*2.7);vec3 color=uBase;
if(uKind<.5){float land=smoothstep(.49,.57,fbm(n*4.1+vec3(3.,1.,2.)));float detail=fbm(n*12.);vec3 ocean=vec3(.008,.055,.12);vec3 landA=vec3(.07,.26,.11);vec3 landB=vec3(.38,.48,.2);color=mix(ocean,landA,land);color=mix(color,landB,smoothstep(.58,.78,detail)*land*.72);float ice=smoothstep(.74,1.,abs(n.y));color=mix(color,vec3(.78,.88,.9),ice*.7);}
else if(uKind<1.5){float bands=.5+.5*sin(n.y*18.+fbm(n*5.)*3.);color=mix(uBase,uAccent,bands*.56);}
else if(uKind<2.5){float bands=.5+.5*sin(n.y*10.+fbm(n*3.)*2.);color=mix(uBase,uAccent,bands*.62);}
else {float bands=.5+.5*sin(n.y*24.+fbm(n*4.)*2.);color=mix(uBase,uAccent,bands*.48);}
color*=.12+.88*wrapped;color+=uAccent*rim*.13;float spec=pow(max(dot(reflect(-light,n),normalize(cameraPosition-vWorldPosition)),0.),52.);color+=vec3(1.)*spec*.3;gl_FragColor=vec4(color,1.);}`;

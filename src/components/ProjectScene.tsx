...
function AuroraArtifact({ intensity }: { intensity: number }) {
  ...
  useFrame(({ clock }) => { if (!group.current) return; const t = clock.getElapsedTime(); group.current.rotation.x = Math.sin(t * 0.45) * 0.16; group.current.rotation.y = t * 0.22; group.current.rotation.z = Math.cos(t * 0.3) * 0.08; });
  return <group ref={group}><mesh><icosahedronGeometry args={[1.15, 2]} /><meshBasicMaterial color="#5eead4" wireframe transparent opacity={0.38 * intensity} /></mesh><mesh scale={0.72}><icosahedronGeometry args={[1.15, 3]} /><meshBasicMaterial color="#8b7cff" wireframe transparent opacity={0.28 * intensity} /></mesh><mesh scale={1.28}><torusGeometry args={[1, 0.018, 8, 80]} /><meshBasicMaterial color="#ffd9a0" transparent opacity={0.5 * intensity} /></mesh></group>;
}
function EchoArtifact({ intensity }: { intensity: number }) {
  const group = useRef<THREE.Group>(null);
  const points = useMemo(() => Array.from({ length: 70 }, (_, i) => { const x = (i / 69 - 0.5) * 3.2; return new THREE.Vector3(x, Math.sin(i * 0.42) * 0.34, Math.cos(i * 0.25) * 0.1); }), []);
  useFrame(({ clock }) => { if (!group.current) return; const t = clock.getElapsedTime(); group.current.rotation.y = Math.sin(t * 0.4) * 0.2; group.current.rotation.z = t * 0.08; group.current.children.forEach((child, i) => { child.position.y = Math.sin(t * 2 + i * 0.42) * 0.08; }); });
...
function LedgerArtifact({ intensity }: { intensity: number }) {
  const group = useRef<THREE.Group>(null);
  useFrame(({ clock }) => { if (!group.current) return; const t = clock.getElapsedTime(); group.current.rotation.y = Math.sin(t * 0.35) * 0.22; group.current.rotation.x = -0.16 + Math.sin(t * 0.25) * 0.04; });
